...
__device__ void REDUCE_FUNC//(void* key, void* vals, size_t keySize, size_t valCount)
{
  ...
  for(int i = 0; i < valCount; i++)
    EMIT_FUNC(key, (PR1_VAL_T*)GET_VAL_FUNC(vals,i), sizeof(L1N_KEY_T), sizeof(L1N_VAL_T));
  return;
#endif 
  ...
  for(int i = 0; i < valCount; i++) {
    float* iVal = (float*)GET_VAL_FUNC(vals, i);
    float cur_val = *iVal;
    sum += cur_val;
  }
  ...
  *o_val = sum;
#ifdef _DEBUG_REDUCE
  printf("reduce: key = %d, value = %f\n", *(int*)key, *o_val);
#endif
  EMIT_FUNC(key, o_val, sizeof(L1N_KEY_T), sizeof(L1N_VAL_T));
}
#endif //__REDUCE_CU__
