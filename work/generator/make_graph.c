...
static void compute_edge_range(int rank, int size, int64_t M,
	...
	*end_idx =
	    (rankc + 1) * (M / sizec) + (rankc + 1 <
					 (M % sizec) ? rankc + 1 : (M % sizec));
}
#endif
...
void make_graph(int log_numverts, int64_t M, uint64_t userseed1,
	...
	generate_kronecker_range(seed, log_numverts, start_idx, end_idx,
				 local_edges);
...
void make_random_numbers(
				 ...
				 */
				int64_t position
				/* Start index in random number stream */ ,
				/*
				 * out 
				 */
				double *result
