...
static int generate_4way_bernoulli(mrg_state * st, int level, int nlevels){
	...
	static const uint32_t limit =
	    (UINT32_C(0x7FFFFFFF) % INITIATOR_DENOMINATOR);
...
#else
	if(val <
	   INITIATOR_A_NUMERATOR * (INITIATOR_DENOMINATOR -
				    2 * INITIATOR_BC_NUMERATOR) /
...
#define USE_GCC_BYTESWAP	/* __builtin_bswap* are in 4.3 but not 4.2 */
	...
	h = (h >> 16) | (h << 16);
	l = (l >> 16) | (l << 16);
	h = ((h >> 8) & UINT32_C(0x00FF00FF)) | ((h & UINT32_C(0x00FF00FF)) <<
						 8);
	l = ((l >> 8) & UINT32_C(0x00FF00FF)) | ((l & UINT32_C(0x00FF00FF)) <<
						 8);
#endif
	h = ((h >> 4) & UINT32_C(0x0F0F0F0F)) | ((h & UINT32_C(0x0F0F0F0F)) <<
						 4);
	l = ((l >> 4) & UINT32_C(0x0F0F0F0F)) | ((l & UINT32_C(0x0F0F0F0F)) <<
						 4);
	h = ((h >> 2) & UINT32_C(0x33333333)) | ((h & UINT32_C(0x33333333)) <<
						 2);
	l = ((l >> 2) & UINT32_C(0x33333333)) | ((l & UINT32_C(0x33333333)) <<
						 2);
	h = ((h >> 1) & UINT32_C(0x55555555)) | ((h & UINT32_C(0x55555555)) <<
						 1);
	l = ((l >> 1) & UINT32_C(0x55555555)) | ((l & UINT32_C(0x55555555)) <<
						 1);
...
#pragma mta block schedule
	...
	for(ei = start_edge; ei < end_edge; ++ei){
		...
		make_one_edge(nverts, 0, logN, &new_state,
			      edges + (ei - start_edge), val0, val1);
	}
}
