...
void make_graph_data_structure(const tuple_graph * const tg)
{
	...
	g_visited =
	    (unsigned long *)xmalloc(visited_size * sizeof(unsigned long));
	g_outgoing =
	    (int64_t *) xMPI_Alloc_mem(coalescing_size * size * 2 *
				       sizeof(int64_t));
	g_outgoing_counts =
	...
	g_recvbuf =
	    (int64_t *) xMPI_Alloc_mem(coalescing_size * 2 * sizeof(int64_t));
}
  ...
  do { \
	...
	*reused. */ \
	...
	while (1) {
		...
		if (num_ranks_done < size) {
			MPI_Irecv(recvbuf, coalescing_size * 2, MPI_INT64_T,
		...
		for (i = 0; i < oldq_count; ++i) {
			...
			size_t j, j_end =
			    g.rowstarts[VERTEX_LOCAL(oldq[i]) + 1];
			for (j = g.rowstarts[VERTEX_LOCAL(oldq[i])]; j < j_end;
			     ++j) {
				...
				if (owner == rank) {
					...
					outgoing[owner * coalescing_size * 2 +
						 c] = tgt;
					outgoing[owner * coalescing_size * 2 +
						 c + 1] = src;
					outgoing_counts[owner] += 2;
					if (outgoing_counts[owner] ==
					    coalescing_size * 2) {
						MPI_Isend(&outgoing
							  [owner *
							   coalescing_size * 2],
		...
		for (offset = 1; offset < size; ++offset) {
			...
			if (outgoing_counts[dest] != 0) {
				...
				MPI_Isend(&outgoing[dest * coalescing_size * 2],
					  outgoing_counts[dest], MPI_INT64_T,
		...
		MPI_Allreduce(&newq_count, &global_newq_count, 1, MPI_INT64_T,
