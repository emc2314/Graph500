...
static void allocate_memory(void)
{
	...
	int64_t local_queue_summary_size =
	    (maxlocalverts + ULONG_BITS * ULONG_BITS -
...
#pragma omp parallel for schedule(static) reduction(|:not_done)
		for (ii_summary = 0; ii_summary < global_queue_summary_size;
		     ++ii_summary) {
			...
			for (ii_offset = 0; ii_offset < ULONG_BITS; ++ii_offset) {
				if ((val_summary & (UINT64_C(1) << ii_offset))
				    == 0)
					continue;
