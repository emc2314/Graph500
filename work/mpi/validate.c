			...
			for(i = i_start; i < i_end; ++i){
				...
				if(p < -1 || p >= nglobalverts){
					fprintf(stderr,
						...
						PRId64 ".\n", rank,
						vertex_to_global_for_pred(rank,
									  i),
						p);
					any_range_errors = 1;
				}
...
#pragma omp parallel for
	...
	gather *pred_win =
	    init_gather((void *)pred, nlocalverts, sizeof(int64_t), pred_pred,
			size_min(CHUNKSIZE, nlocalverts), size_min(CHUNKSIZE,
								   nlocalverts),
			MPI_INT64_T);
...
#pragma omp parallel for
				...
				get_vertex_distribution_for_pred(i_end -
								 i_start,
								 pred_vtx,
								 pred_owner,
								 pred_local);
						...
						if(get_depth_from_pred_entry
						   ...
						   1){
							fprintf(stderr,
								...
								PRIu16 ").\n",
								rank,
								vertex_to_global_for_pred
								(rank, i),
								get_depth_from_pred_entry
								(pred[i]),
								get_pred_from_pred_entry
								(pred[i]),
								get_depth_from_pred_entry
								(pred_pred
								 [i -
		...
		if(root_is_mine &&
		   get_depth_from_pred_entry(pred[root_local]) != 0){
			fprintf(stderr,
				...
				get_depth_from_pred_entry(pred[root_local]));
			validation_passed = 0;
		}
			...
			if(get_pred_from_pred_entry(pred[i]) == -1 &&
			   get_depth_from_pred_entry(pred[i]) != UINT16_MAX){
				...
				fprintf(stderr,
					...
					rank, vertex_to_global_for_pred(rank,
									i),
					get_pred_from_pred_entry(pred[i]));
				validation_passed = 0;
			}
			...
			if(get_depth_from_pred_entry(pred[i]) !=
			   ...
			   1){
				fprintf(stderr,
					...
					get_depth_from_pred_entry(pred[i]),
					get_pred_from_pred_entry(pred[i]),
					get_depth_from_pred_entry(pred_pred
								  [i -
								   i_start]));
				validation_passed = 0;
			}
	...
	if(root_is_mine){
		...
		if(get_pred_from_pred_entry(pred[root_local]) != root){
			fprintf(stderr,
				...
				PRId64 " is %" PRId64
				", not the root itself.\n", rank, root,
				get_pred_from_pred_entry(pred[root_local]));
			validation_passed = 0;
		}
	}
...
#pragma omp parallel for
			...
			get_vertex_distribution_for_pred(i_end - i_start,
							 pred_vtx, pred_owner,
							 pred_local);
#pragma omp parallel for reduction(&&:validation_passed)
			for(i = i_start; i < i_end; ++i){
...

	{
		...
		int64_t *restrict edge_preds =
		    (int64_t *) xMPI_Alloc_mem(2 * edge_chunk_size *
					       sizeof(int64_t));
		gather *pred_win =
		    init_gather((void *)pred, nlocalverts, sizeof(int64_t),
				edge_preds, 2 * edge_chunk_size,
				2 * edge_chunk_size, MPI_INT64_T);
		unsigned char one = 1;
		scatter_constant *pred_valid_win =
		    init_scatter_constant((void *)pred_valid, nlocalverts,
					  sizeof(unsigned char), &one,
					  2 * edge_chunk_size,
					  MPI_UNSIGNED_CHAR);
...
#pragma omp parallel for
				...
				get_vertex_distribution_for_pred(2 *
								 (i_end -
								  i_start),
								 edge_endpoint,
								 edge_owner,
...
#pragma omp parallel for reduction(&&:validation_passed) reduction(+:edge_visit_count)
				for(i = i_start; i < i_end; ++i){
					...
					uint16_t src_depth =
					    get_depth_from_pred_entry(edge_preds
								      [(i -
									i_start)
								       * 2 +
								       0]);
...
#pragma omp parallel for reduction(&&:validation_passed)
		...
		MPI_Allreduce(MPI_IN_PLACE, &edge_visit_count, 1, MPI_INT64_T,
