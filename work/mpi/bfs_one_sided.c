...
void run_bfs(int64_t root, int64_t * pred){
	...
	unsigned long *queue_bitmap1 =
	    (unsigned long *)xMPI_Alloc_mem(queue_nwords *
	...
	if(VERTEX_OWNER(root) == rank){
		...
		queue_bitmap1[VERTEX_LOCAL(root) / elts_per_queue_bit /
			      ulong_bits] |=
		    (1UL <<
		     ((VERTEX_LOCAL(root) / elts_per_queue_bit) % ulong_bits));
	}
	...
	while(1){
		...
		for(i = 0; i < queue_nwords; ++i){
			...
			for(bitnum = 0; bitnum < ulong_bits; ++bitnum){
				...
				for(qelem_idx = 0;
				    qelem_idx < elts_per_queue_bit;
				    ++qelem_idx){
					size_t v_local =
					    first_v_local + qelem_idx;
					...
					if(pred[v_local] >= 0 &&
					   pred[v_local] < nglobalverts){
						size_t ei, ei_end =
						    g.rowstarts[v_local + 1];
						/*
						 * Walk the incident edges. 
						 */
						for(ei = g.rowstarts[v_local];
						    ei < ei_end; ++ei){
							int64_t w =
							    g.column[ei];
							if(w ==
							 ...
							 * * way as well). 
							 */
							MPI_Accumulate(&masks
								       [((VERTEX_LOCAL(w) / elts_per_queue_bit) % ulong_bits)], 1, MPI_UNSIGNED_LONG, VERTEX_OWNER(w), VERTEX_LOCAL(w) / elts_per_queue_bit / ulong_bits, 1, MPI_UNSIGNED_LONG, MPI_BOR, queue2_win);
						}
					}
				}
			}
		...
		MPI_Allreduce(MPI_IN_PLACE, &any_set, 1, MPI_INT, MPI_LOR,
			      MPI_COMM_WORLD);
		if(!any_set)
			break;
