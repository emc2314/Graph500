...
#pragma omp parallel for
		for (i = 0; i < (ptrdiff_t) inbuf_size; ++i) {
			assert((size_t)
				...
				ULONG_BITS) < nrows);
#pragma omp atomic
			++counts[SWIZZLE_VERTEX(get_v1_from_edge(&inbuf[i])) /
				 ULONG_BITS];
		}
		rowstarts[0] = 0;
		for (i = 0; i < nrows; ++i) {
...
#pragma omp parallel for
		for (i = 0; i < (ptrdiff_t) inbuf_size; ++i) {
			...
			size_t pos =
			    __sync_fetch_and_add(&inserts[(v1) / ULONG_BITS],
						 1);
			column[pos] =
			    (v1 % ULONG_BITS) + VERTEX_LOCAL(v0) * ULONG_BITS;
		      ...
		      temp_csc_graph * restrict const a)
{
	if (b->lg_local_queue_size == -1) {	// b is empty
		...
		*b = *a;
		 ...
		 */
		fprintf(stderr,
			"%d: a->nglobalverts=%" PRId64 " != b->nglobalverts=%"
		...
		if (b_nlocaledges + a_nlocaledges > b->nlocaledges_allocated) {
			...
			b->column =
			    (int64_t *) xrealloc(b->column,
						 new_alloc * sizeof(int64_t));
		}
		ptrdiff_t i_plus_1;
...
static MAKE_REDISTRIBUTE_FUNC(CONV1D_FUNCNAME, CONV1D_EXTRA_PARAMS,
			      CONV1D_DECLARE_AND_INIT_GRAPH_SO_FAR,
			      CONV1D_CALL_ON_EDGES, CONV1D_EDGE_BUFFER_TYPE,
			      CONV1D_EDGE_BUFFER_MPI_TYPE,
			      CONV1D_PRECOMPRESS_INCOMING_DATA,
			      CONV1D_MERGE_INTO_GRAPH_SO_FAR,
			       ...
			       oned_csc_graph * const g)
{
	...
	int64_t local_queue_summary_size =
	    (g->max_nlocalverts + ULONG_BITS * ULONG_BITS -
