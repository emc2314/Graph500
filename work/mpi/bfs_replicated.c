...
static void allocate_memory(void){
	...
	int lg_local_queue_size =
	    lg_int64_t((maxlocalverts + ulong_bits_squared -
			1) / ulong_bits_squared * ulong_bits_squared);
	g_lg_local_queue_size = lg_local_queue_size;
	int64_t local_queue_summary_size =
	    (INT64_C(1) << lg_local_queue_size) / ulong_bits_squared;
	...
	g_in_queue_summary =
	    (unsigned long *)xmalloc(global_queue_summary_size *
				     sizeof(unsigned long));
	g_out_queue =
	    (unsigned long *)xmalloc(local_queue_size * sizeof(unsigned long));
	g_out_queue_summary =
	    (unsigned long *)xmalloc(local_queue_summary_size *
				     sizeof(unsigned long));
	g_visited =
	    (unsigned long *)xmalloc(local_queue_size * sizeof(unsigned long));
}
...
#if 0
	int64_t *restrict column_swizzled =
	    (int64_t *) xmalloc(nlocaledges * sizeof(int64_t));
	{
		size_t i;
...
#define SET_VISITED_LOCAL(v) do {size_t word_idx = (v) / ulong_bits; int bit_idx = (v) % ulong_bits; unsigned long mask = (1UL << bit_idx); visited[word_idx] |= mask; out_queue[word_idx] |= mask;} while (0)
	...
	SET_IN(root);
	{
		...
		_Pragma("omp parallel for schedule(static)") for(i = 0;
								 i <
								 nlocalverts;
								 ++i)
			pred[i] = -1;
...
#if 0
		if(rank == 0)
			fprintf(stderr, "BFS level %" PRIu16 "\n", cur_level);
#endif
		memset(out_queue, 0,
		       (nlocalverts + ulong_bits -
			1) / ulong_bits * sizeof(unsigned long));
...
#endif
		MPI_Allreduce(MPI_IN_PLACE, &not_done, 1, MPI_UNSIGNED_LONG,
			      MPI_BOR, MPI_COMM_WORLD);
		if(not_done == 0)
			break;
		MPI_Allgather(out_queue, local_queue_size, MPI_UNSIGNED_LONG,
