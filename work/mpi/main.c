...
enum { s_minimum, s_firstquartile, s_median, s_thirdquartile, s_maximum, s_mean,
	    s_std, s_LAST };
...
int main(int argc, char **argv){
	...
	if(tg.data_in_file){
		...
		if(!reuse_file){
			...
			MPI_File_set_errhandler(MPI_FILE_NULL,
						MPI_ERRORS_RETURN);
			if(MPI_File_open
			   (MPI_COMM_WORLD, (char *)filename, mode,
			    MPI_INFO_NULL, &tg.edgefile)){
				if(0 == rank && getenv("VERBOSE"))
					fprintf(stderr,
						"%d: failed to open %s, creating\n",
						rank, filename);
				...
				if(size ==
				   tg.nglobaledges * sizeof(packed_edge)){
					is_opened = 1;
					tg.write_file = 0;
		...
		if(!is_opened){
			MPI_File_open(MPI_COMM_WORLD, (char *)filename, mode,
				      MPI_INFO_NULL, &tg.edgefile);
	...
	int64_t *bfs_roots =
	    (int64_t *) xmalloc(num_bfs_roots * sizeof(int64_t));
...

	{
		...
		int64_t bitmap_size_in_bytes =
		    int64_min(BITMAPSIZE,
			      (nglobalverts + CHAR_BIT - 1) / CHAR_BIT);
		if(bitmap_size_in_bytes * size * CHAR_BIT < nglobalverts){
			bitmap_size_in_bytes =
			    (nglobalverts + size * CHAR_BIT -
			     1) / (size * CHAR_BIT);
		}
		int ranks_per_row =
		    ((nglobalverts + CHAR_BIT - 1) / CHAR_BIT +
		     bitmap_size_in_bytes - 1) / bitmap_size_in_bytes;
		...
		if(cart_comm != MPI_COMM_NULL){
			in_generating_rectangle = 1;
			{
				...
				MPI_Cart_get(cart_comm, 2, dims, periods,
					     coords);
				my_row = coords[0];
				my_col = coords[1];
			}
			...
			has_edge =
			    (unsigned char *)
			    xMPI_Alloc_mem(bitmap_size_in_bytes);
			 ...
			 */
			packed_edge *buf =
			    (packed_edge *) xmalloc(FILE_CHUNKSIZE *
						    sizeof(packed_edge));
			MPI_Offset block_limit =
			    (nchunks_in_file + nrows - 1) / nrows;
			/*
			...
			if(tg.data_in_file){
				...
				int last_pos =
				    (tg.nglobaledges %
				     ((int64_t) FILE_CHUNKSIZE * nrows *
				      ranks_per_row) !=
				     0) ? (tg.nglobaledges / FILE_CHUNKSIZE) %
				    (nrows * ranks_per_row) : -1;
				int64_t edges_left =
				    tg.nglobaledges % FILE_CHUNKSIZE;
				int64_t nedges =
				    FILE_CHUNKSIZE * (tg.nglobaledges /
						      ((int64_t) FILE_CHUNKSIZE
						       * nrows *
						       ranks_per_row)) +
				    FILE_CHUNKSIZE * (my_pos <
						      (tg.nglobaledges /
						       FILE_CHUNKSIZE) %
						      (nrows * ranks_per_row)) +
			...
			for(block_idx = 0; block_idx < block_limit; ++block_idx){
				 ...
				 */
				MPI_Offset start_edge_index =
				    int64_min(FILE_CHUNKSIZE *
					      (block_idx * nrows + my_row),
					      tg.nglobaledges);
				MPI_Offset edge_count =
				    int64_min(tg.nglobaledges -
					      start_edge_index, FILE_CHUNKSIZE);
				packed_edge *actual_buf = (!tg.data_in_file &&
							   block_idx %
							   ranks_per_row ==
							   my_col) ? tg.
				    edgememory +
				    FILE_CHUNKSIZE * (block_idx /
						      ranks_per_row) : buf;
				/*
				 ...
				 */
				if(!tg.data_in_file &&
				   block_idx % ranks_per_row == my_col){
					assert(FILE_CHUNKSIZE *
					       (block_idx / ranks_per_row) +
					       edge_count <=
					       tg.edgememory_size);
				}
				if(tg.write_file){
					 ...
					 */
					MPI_File_read_at(tg.edgefile,
							 start_edge_index,
							 actual_buf, edge_count,
							 packed_edge_mpi_type,
							 MPI_STATUS_IGNORE);
				}
				ptrdiff_t i;
...
#pragma omp parallel for
				...
				for(i = 0; i < edge_count; ++i){
					int64_t src =
					    get_v0_from_edge(&actual_buf[i]);
					int64_t tgt =
					    get_v1_from_edge(&actual_buf[i]);
					if(src == tgt)
						continue;
					if(src / bitmap_size_in_bytes /
					   CHAR_BIT == my_col){
#ifdef _OPENMP
#pragma omp atomic
#endif
						has_edge[(src / CHAR_BIT) %
							 bitmap_size_in_bytes]
						    |= (1 << (src % CHAR_BIT));
					}
					if(tgt / bitmap_size_in_bytes /
					   CHAR_BIT == my_col){
#ifdef _OPENMP
#pragma omp atomic
#endif
						has_edge[(tgt / CHAR_BIT) %
							 bitmap_size_in_bytes]
						    |= (1 << (tgt % CHAR_BIT));
					}
				}
			 ...
			 */
			MPI_Allreduce(MPI_IN_PLACE, has_edge,
				      bitmap_size_in_bytes, MPI_UNSIGNED_CHAR,
				      MPI_BOR, this_col);
#endif
		...
		MPI_Allreduce(&tg.edgememory_size, &tg.max_edgememory_size, 1,
		 ...
		 */
		{
			...
			for(bfs_root_idx = 0; bfs_root_idx < num_bfs_roots;
			    ++bfs_root_idx){
				int64_t root;
				...
				while(1){
					...
					make_random_numbers(2, seed1, seed2,
							    counter, d);
					root =
					    (int64_t)((d[0] +
							d[1]) * nglobalverts) %
					...
					MPI_Allreduce(MPI_IN_PLACE, &root_ok, 1,
						      MPI_INT, MPI_LOR,
						      MPI_COMM_WORLD);
					if(root_ok)
						break;
			 ...
			 */
			{
				...
				max_used_vertex = 0;
				if(in_generating_rectangle){
					for(i = bitmap_size_in_bytes * CHAR_BIT;
					    i > 0; --i){
						if(i > nglobalverts)
							continue;
						if(has_edge[(i - 1) / CHAR_BIT]
						   & (1 <<
						      ((i - 1) % CHAR_BIT))){
							max_used_vertex =
							    (i - 1) +
				...
				MPI_Allreduce(MPI_IN_PLACE, &max_used_vertex, 1,
					      MPI_INT64_T, MPI_MAX,
					      MPI_COMM_WORLD);
			}
		}
	...
	double *validate_times =
	    (double *)xmalloc(num_bfs_roots * sizeof(double));
	...
	for(bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx){
		...
		if(!getenv("SKIP_VALIDATION")){
			...
			int validation_passed_one =
			    validate_bfs_result(&tg, max_used_vertex + 1,
						nlocalverts, root, pred,
						&edge_visit_count);
			double validate_stop = MPI_Wtime();

			validate_times[bfs_root_idx] =
			    validate_stop - validate_start;
			if(rank == 0)
				fprintf(stderr,
					"Validate time for BFS %d is %f\n",
					bfs_root_idx,
					validate_times[bfs_root_idx]);
	...
	if(rank == 0){
		if(!validation_passed){
			...
			fprintf(stdout,
				...
				stats[s_minimum]);
			fprintf(stdout,
				"firstquartile_nedge:            %.11g\n",
				stats[s_firstquartile]);
			fprintf(stdout,
				"median_nedge:                   %.11g\n",
				stats[s_median]);
			fprintf(stdout,
				"thirdquartile_nedge:            %.11g\n",
				stats[s_thirdquartile]);
			fprintf(stdout,
				"max_nedge:                      %.11g\n",
				stats[s_maximum]);
			...
			fprintf(stdout, "harmonic_stddev_TEPS:           %g\n",
				stats[s_std] / (stats[s_mean] * stats[s_mean] *
						sqrt(num_bfs_roots - 1)));
			free(secs_per_edge);
			secs_per_edge = NULL;
			free(edge_counts);
			edge_counts = NULL;
